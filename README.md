...
一个基于 Flask 的全功能用户信息管理平台，支持**登录、注册、个人中心、余额充值、头像上传、用户搜索、修改密码、URL 抓取、Ping 测试、XML 导入、动态页面加载**等 **11 项功能**。项目经过**完整安全审计与加固**，修复了包括 XXE、命令注入、SSRF、SQL 注入、路径遍历、越权访问在内的 **22 项安全漏洞**，修复率 **100%**。
...
| 📡 Ping 诊断 | `GET/POST /ping` | Ping 网络连通性测试（防命令注入） | 登录 |
| 📄 XML 导入 | `GET/POST /xml-import` | 解析 XML 数据并返回 JSON（防 XXE 攻击） | 登录 |
...
| 20 | URL 抓取 SSRF / file 协议 | 🔴 严重 | 协议白名单 + `ipaddress` 校验（禁内网/回环） | ✅ |
| 21 | Ping 命令注入 | 🔴 严重 | `shlex.quote()` 转义用户输入 | ✅ |
| 22 | XML 外部实体注入（XXE） | 🔴 严重 | 移除 DOCTYPE + 检测实体引用 + 拒绝执行 | ✅ |
...
| `GET/POST` | `/xml-import` | XML 导入 | ✅ | - | 防 XXE 攻击 |
| `GET` | `/page` | 动态页面 | - | - | 路径约束 |
...
XML 导入功能通过**移除 DOCTYPE 声明**和**检测实体引用来**防止 XXE 攻击：

```python
# 修复前（危险）：
with open(filepath, 'r') as f:     # 读取任意本地文件
    xml_data = f.read()

# 修复后（安全）：
safe_xml = re.sub(r'<!DOCTYPE.*?>', '', xml_data)  # 移除 DOCTYPE
if re.search(r'&[a-z]+;', safe_xml):               # 检测实体引用
...
# 7. XML 导入
curl -b cookies.txt -X POST http://127.0.0.1:5000/xml-import \
  -d 'xml_data=<users><user><name>Alice</name><email>alice@test.com</email></user></users>'
