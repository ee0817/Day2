...
一个基于 Flask 的用户信息管理平台，支持**登录、注册、个人中心、充值、头像上传、用户搜索**功能。经过完整安全审计与加固，修复了 16 项安全漏洞。
...
| 功能 | 说明 | 权限 |
|:-----|:------|:----:|
| 🔑 登录 | 用户登录（含登录限流、CSRF 保护） | 公开 |
| 📝 注册 | 新用户注册（含输入过滤与密码强度校验） | 公开 |
| 👤 个人中心 | 查看本人资料（ID、用户名、邮箱、手机、余额） | 登录 |
| 💰 充值 | 为当前登录账号充值 | 登录 |
| 📷 头像上传 | 上传用户头像 | 登录 |
...
| 15 | 越权访问个人中心 | `user_id` 必须与当前登录用户匹配 | ✅ |
| 16 | 越权充值 / 负值充值 | 只能给自己充值 + 金额必须为正数 | ✅ |
...
| GET | `/profile` | 个人中心（查看本人资料） | 登录 | - |
| POST | `/recharge` | 充值（仅限本人） | 登录 | - |
